...
const startOfMonth=(0,_moment.default)(date).startOf("month");const endOfMonth=(0,_moment.default)(date).endOf("month");// Adjust startOfMonth if it's after the provided date.
if(startOfMonth.isAfter((0,_moment.default)(date))){startOfMonth.subtract(1,"week")}// Initialize the array to store days.
...
let i=0;while(true){if(currentDay.isAfter(endOfMonth)&&i%7===0){break}days.push({index:currentDay.format("DDMMYYYY"),date:currentDay.clone(),isInCurrentMonth:currentDay.isSame(date,"month"),isCurrentDay:currentDay.isSame(new Date,"day")});currentDay.add(1,"day");i++}// Reverse the order of days if right-to-left is enabled.
