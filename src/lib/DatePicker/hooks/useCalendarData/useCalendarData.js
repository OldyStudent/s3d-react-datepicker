 ...
 */
const useCalendarData = (currentDate, options) => {
  // Set the locale for moment.js based on the provided language option.
  moment.locale(options.lang);

  // Generate day name list using useMemo for memoization.
  const dayNameList = useMemo(() => generateDayOfWeekNames(options), [options]);

  // Generate month name list using useMemo for memoization.
  const monthNameList = useMemo(() => generateMonthNames(options), [options]);

  // Generate year list using useMemo for memoization.
  const yearList = useMemo(() => generateYearList(options), [options]);

  // Generate calendar day list using useMemo for memoization.
  const calendarDayList = useMemo(
    () =>
      getCalendarMonthDays(currentDate, options.dayOfWeekStart, options.rtl),
    [currentDate, options],
  );

  // Return the generated calendar data.
 ...
 */
function generateDayOfWeekNames(options) {
  const firstDayOfWeek = options.dayOfWeekStart;
  const days = [];

  for (let j = 0; j < 7; j += 1) {
    const dayIndex = (j + firstDayOfWeek) % 7;
    days.push({
      index: dayIndex,
      name: options.i18n[options.lang].dayOfWeekShort[dayIndex],
    });
  }
 ...
 */
function generateMonthNames(options) {
  return options.i18n[options.lang].months.map((month, index) => ({
    index,
    value: month,
  }));
}
 ...
 */
function generateYearList(options) {
  const years = [];

  for (let i = options.yearStart; i <= options.yearEnd; i++) {
    years.push({ index: i, value: i });
  }
 ...
 */
function getCalendarMonthDays(date, firstDayOfWeek = 0, rightToLeft = false) {
  // Ensure firstDayOfWeek is within the range [0, 6].
  ...
  while (true) {
    ...
    days.push({
      ...
      isInCurrentMonth: currentDay.isSame(date, "month"),
      isCurrentDay: currentDay.isSame(new Date(), "day"),
    });
  ...
  if (rightToLeft) {
    for (let i = 0; i < days.length; i += 7) {
      days.splice(i, 7, ...days.slice(i, i + 7).reverse());
    }
  }
